...
def test_supported_pairs_csv_export():
    supported_pairs = []
    ...
    # Read the CSV file and check if it has the correct format
    with open("supported_pairs.csv", "r") as f:
        reader = csv.DictReader(f)
        for row in reader:
            assert "source language" in row
            assert "target language" in row
            assert "translation engine" in row
            assert "is preferred engine?" in row
